...
def generate_and_combine_heatmaps(model, dataloader, save_dir):
    # Generate and save individual heatmaps
    heatmap_paths = []
    for i, (images, _) in enumerate(dataloader):
        for j in range(images.shape[0]):
            image = images[j]
            image_name = f'image_{i * dataloader.batch_size + j}'
            generate_individual_heatmap(model, image, save_dir, image_name)
            heatmap_paths.append(os.path.join(save_dir, f'{image_name}_heatmap.png'))

    # Combine individual heatmaps into one
    combine_heatmaps(heatmap_paths, save_dir)
